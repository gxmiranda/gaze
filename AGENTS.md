...
- provider-cleanup: Removed deprecated `ContractCoverageFunc` and `SSADegradedPackages` fields from `crap.Options`, eliminating the fallback dispatch path in `crap.Analyze`. Contract coverage now flows exclusively through `ContractCoverageProvider`. Deduplicated `isMainPkg` in `goprovider/sideeffect.go` by replacing local copy with `loader.IsMainPkg`. Relocated `BuildContractCoverageFunc` and helpers (`analyzePackageCoverage`, `loadTestPackage`, `classifyResults`, `loadGazeConfigBestEffort`) from `internal/crap/contract.go` to `internal/provider/goprovider/contract.go`, achieving zero Go-specific imports in `internal/crap/`. Generalized taxonomy: renamed `Metadata.GoVersion` to `LanguageVersion` (JSON tag `language_version`), added `Language` field (JSON tag `language`), updated JSON schemas. Added 7 language-neutral `SideEffectType` aliases (`AsyncTaskSpawn`, `AsyncMessageSend`, `AsyncChannelClose`, `BarrierOp`, `PanicRecovery`, `FFICall`, `ObjectPoolOp`). Added streaming protocol mode: `Streaming` capability flag, `analyze/stream` method constant, `CallStream` on protocol client, streaming support in `ExternalSideEffectAnalyzer`, fake analyzer `--hang-stream` mode. Added `LanguageVersion` to `protocol.InitializeResult` and `adapter.Providers`. JSON schema breaking change: `go_version` → `language_version`.
